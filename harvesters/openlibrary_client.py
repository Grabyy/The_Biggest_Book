        ...
        hits.append(
            {
                ...
                "subjects": (d.get("subject") or [])[:8],  # keep it short for UI
...
def build_payload_from_title_hit(hit: dict) -> dict:
    ...
    - Use subjects from the search hit (no extra call to the work endpoint).
    """
    ...
    return {
        ...
        "subjects": hit.get("subjects") or [],   # <- from search hit only (no extra HTTP)
